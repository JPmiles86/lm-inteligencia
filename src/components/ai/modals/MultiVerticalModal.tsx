// Multi-Vertical Modal - Generate content for multiple industries simultaneously
...
interface MultiVerticalModalProps {
  isOpen: boolean;
  onClose: () => void;
}
...
export const MultiVerticalModal: React.FC<MultiVerticalModalProps> = ({
  isOpen,
  onClose,
}) => {
  const {
...
Please adapt the content specifically for the ${vertical.name} industry, using relevant examples, terminology, and addressing specific challenges faced by ${vertical.targetAudience}.`;
  ...
  const getStatusDisplay = (status: VerticalConfig['status']) => {
    switch (status) {
      case 'generating':
        return { icon: Loader2, className: 'animate-spin text-blue-600', label: 'Generating...' };
      case 'completed':
        return { icon: Check, className: 'text-green-600', label: 'Completed' };
      case 'error':
        return { icon: X, className: 'text-red-600', label: 'Error' };
      default:
        return { icon: Globe, className: 'text-gray-400', label: 'Pending' };
    }
  };
                      ...
                      <div className="p-4">
                        <div className="flex items-center justify-between">
                          <label className="flex items-center space-x-3 cursor-pointer">
                            ...
                            />
                            <IconComponent className={`h-5 w-5 ${vertical.color.split(' ')[0]}`} />
                            <div>
                      ...
                      return (
                        ...
                        >
                          <div className="flex items-center justify-between mb-3">
                            <div className="flex items-center space-x-2">
                              <IconComponent className={`h-4 w-4 ${vertical.color.split(' ')[0]}`} />
                              <span className="font-medium text-gray-900 dark:text-white">
                                {vertical.name}
                              </span>
                            </div>

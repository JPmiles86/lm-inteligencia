// Context Selection Content - Extracted from ContextSelectionModal
// Responsive content component for context selection within unified modal
...
interface ContextSelectionContentProps {
  activeVertical?: string;
  onClose?: () => void;
  isMobile?: boolean;
}

export const ContextSelectionContent: React.FC<ContextSelectionContentProps> = ({
  ...
  useEffect(() => {
    loadContextData();
    buildPreview();
    // Set initial value for custom context
  ...
  const tabs = [
    { id: 'guides', label: 'Style Guides', icon: Building },
    { id: 'content', label: 'Previous Content', icon: Tag },
    ...
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className={`flex ${isMobile ? 'flex-col' : 'flex-row'} h-full overflow-hidden`}>
        ...
        <div className={`${isMobile ? 'flex-1' : 'flex-1'} flex flex-col overflow-hidden`}>
          ...
          <div className={`flex border-b border-gray-200 dark:border-gray-700 ${isMobile ? 'overflow-x-auto' : ''}`}>
            {tabs.map((tab) => {
              ...
              return (
                <button
                  ...
                  className={`flex items-center space-x-2 px-4 md:px-6 py-3 text-sm font-medium border-b-2 transition-colors whitespace-nowrap ${
                    activeTab === tab.id
          ...
          {(activeTab === 'guides' || activeTab === 'content') && (
            <div className="p-4 border-b border-gray-200 dark:border-gray-700">
              <div className={`flex items-center space-y-2 ${isMobile ? 'flex-col space-y-2 space-x-0' : 'flex-row space-y-0 space-x-4'}`}>
                <div className={`relative ${isMobile ? 'w-full' : 'flex-1'}`}>
                  ...
                  <input
                    ...
                    className="w-full pl-10 pr-4 py-2 text-sm border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                  />
                </div>
                ...
                {activeTab === 'content' && (
                  <select
                    ...
                    className={`px-3 py-2 text-sm border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-blue-500 ${isMobile ? 'w-full' : ''}`}
                  >
        ...
        {/* Right Panel - Preview (Hidden on mobile, show as separate section) */}
        {!isMobile && (
      ...
      {/* Mobile: Context preview as collapsible section */}
      {isMobile && previewContext && (
        <div className="border-t border-gray-200 dark:border-gray-700 p-4">
          <div className="bg-blue-50 dark:bg-blue-900 rounded p-3">
            ...
            <div className="text-xs text-blue-700 dark:text-blue-200">
              <p>Quality: {Object.values(selectedContext.styleGuides).flat().filter(Boolean).length > 0 ||
               selectedContext.previousContent.mode !== 'none' ||
               selectedContext.additionalContext
                ? 'High' : 'Basic'} | Tokens: ~{Math.ceil((previewContext?.length || 0) / 4)}</p>
            </div>
          </div>
        </div>
      )}
